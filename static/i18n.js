...
const LOCALES = {
  en: {
    ...
    cmd_skills: 'List available AVOI skills',
  ...
  folder_add_as_space_title: 'Add as Space?',
    ...
    settings_section_preferences_meta: 'Defaults and UI behavior for AVOI WebUI.',
    ...
    settings_desc_cli_sessions: 'Merges sessions from the AVOI CLI (state.db) into the session list. Click a CLI session to import it and continue the conversation.',
    ...
    settings_desc_bot_name: 'Display name for the assistant throughout the UI. Defaults to AVOI.',
    ...
    providers_oauth_config_yaml_hint: 'Token configured via config.yaml. To update, edit the providers section in your config.yaml or run avoi auth.',
    providers_oauth_not_configured_hint: 'Not authenticated. Run avoi auth in the terminal to configure this provider.',
    providers_save: 'Save',
    ...
    onboarding_title: 'Welcome to AVOI WebUI',
    onboarding_lead: 'A quick guided setup will verify AVOI, save a real provider configuration, choose a workspace and model, and optionally protect the app with a password.',
    onboarding_back: 'Back',
    onboarding_continue: 'Continue',
    onboarding_skip: 'Skip setup',
    onboarding_skipped: 'Setup skipped — using existing config.',
    onboarding_open: 'Open AVOI',
    onboarding_step_system_title: 'System check',
    onboarding_step_system_desc: 'Verify AVOI Agent and config visibility.',
    onboarding_step_setup_title: 'Provider setup',
    onboarding_step_setup_desc: 'Save the minimum AVOI provider config.',
    ...
    onboarding_notice_system_ready: 'AVOI Agent looks reachable from the Web UI.',
    onboarding_notice_system_unavailable: 'AVOI Agent is not fully available yet. Bootstrap can install it, but provider setup may still require a terminal.',
    onboarding_check_agent: 'AVOI Agent',
    ...
    onboarding_notice_setup_required: 'Choose a simple provider path here. Advanced OAuth flows still belong in the AVOI CLI for now.',
    onboarding_notice_setup_already_ready: 'A working AVOI provider setup is already detected. You can keep it or replace it here.',
    onboarding_oauth_provider_ready_title: 'Provider already authenticated',
    onboarding_oauth_provider_ready_body: 'This instance is configured to use an OAuth provider (<strong>{provider}</strong>) that was set up via the AVOI CLI. No API key is needed here — click Continue to finish setup.',
    onboarding_oauth_provider_not_ready_title: 'OAuth provider not yet authenticated',
    onboarding_oauth_provider_not_ready_body: 'This instance is configured to use <strong>{provider}</strong>, which uses OAuth rather than an API key. Run <code>avoi auth</code> or <code>avoi model</code> in a terminal to authenticate, then reload the Web UI.',
    ...
    onboarding_api_key_help_prefix: 'Saved as a secret in your AVOI .env file using',
    ...
    onboarding_workspace_help: 'Pick the model AVOI should use for new chats after setup completes.',
  ...
  ru: {
    ...
    cmd_skills: 'Показать доступные навыки AVOI',
  ...
  folder_add_as_space_title: 'Добавить как пространство?',
    ...
    settings_desc_cli_sessions: 'Объединяет сеансы из AVOI CLI (state.db) в список сеансов. Нажмите на CLI-сеанс, чтобы импортировать его и продолжить разговор.',
    settings_desc_sync_insights: 'Синхронизирует использование токенов WebUI в state.db, чтобы AVOI /insights включал данные браузерных сеансов. Выключено по умолчанию.',
    ...
    settings_desc_bot_name: 'Отображаемое имя помощника во всём интерфейсе. По умолчанию AVOI.',
    ...
    onboarding_title: 'Добро пожаловать в AVOI WebUI',
    onboarding_lead: 'Краткая пошаговая настройка проверит AVOI, сохранит рабочую конфигурацию провайдера, выберет рабочее пространство и модель и при желании защитит приложение паролем.',
    ...
    onboarding_open: 'Открыть AVOI',
    onboarding_step_system_title: 'Проверка системы',
    onboarding_step_system_desc: 'Проверить AVOI Agent и видимость конфигурации.',
    onboarding_step_setup_title: 'Настройка провайдера',
    onboarding_step_setup_desc: 'Сохранить минимальную рабочую конфигурацию провайдера AVOI.',
    ...
    onboarding_notice_system_ready: 'AVOI Agent, похоже, доступен из Web UI.',
    onboarding_notice_system_unavailable: 'AVOI Agent ещё не полностью доступен. Bootstrap может установить его, но для настройки провайдера всё ещё может понадобиться терминал.',
    onboarding_check_agent: 'AVOI Agent',
    ...
    onboarding_notice_setup_required: 'Выберите здесь простой путь настройки провайдера. Продвинутые OAuth-сценарии пока остаются в AVOI CLI.',
    onboarding_notice_setup_already_ready: 'Уже обнаружена рабочая конфигурация провайдера AVOI. Вы можете оставить её или заменить здесь.',
    onboarding_oauth_provider_ready_title: 'Провайдер уже авторизован',
    onboarding_oauth_provider_ready_body: 'Этот экземпляр настроен на использование OAuth-провайдера (<strong>{provider}</strong>), настроенного через AVOI CLI. API-ключ здесь не нужен — нажмите «Продолжить», чтобы завершить настройку.',
    onboarding_oauth_provider_not_ready_title: 'OAuth-провайдер ещё не авторизован',
    onboarding_oauth_provider_not_ready_body: 'Этот экземпляр настроен на использование <strong>{provider}</strong>, который работает через OAuth, а не через API-ключ. Запустите <code>avoi auth</code> или <code>avoi model</code> в терминале, чтобы пройти авторизацию, затем обновите Web UI.',
    ...
    onboarding_api_key_help_prefix: 'Сохраняется как секрет в вашем файле `.env` AVOI с помощью',
    ...
    onboarding_workspace_help: 'Выберите модель, которую AVOI должен использовать для новых чатов после завершения настройки.',
  ...
  es: {
    ...
    cmd_skills: 'Listar las skills de AVOI disponibles',
  ...
  folder_add_as_space_title: '¿Añadir como espacio?',
    ...
    settings_desc_cli_sessions: 'Fusiona las sesiones del CLI de AVOI (state.db) en la lista de sesiones. Haz clic en una sesión de CLI para importarla y continuar la conversación.',
    ...
    settings_desc_bot_name: 'Nombre visible del asistente en toda la UI. Por defecto es AVOI.',
    ...
    onboarding_title: 'Bienvenido a AVOI WebUI',
    onboarding_lead: 'Una guía rápida verificará AVOI, guardará una configuración real del proveedor, elegirá un espacio de trabajo y un modelo, y opcionalmente protegerá la app con una contraseña.',
    ...
    onboarding_open: 'Abrir AVOI',
    onboarding_step_system_title: 'Comprobación del sistema',
    onboarding_step_system_desc: 'Verifica AVOI Agent y la visibilidad de la configuración.',
    onboarding_step_setup_title: 'Configuración del proveedor',
    onboarding_step_setup_desc: 'Guarda la configuración mínima real de AVOI.',
    ...
    onboarding_notice_system_ready: 'Parece que AVOI Agent está accesible desde la Web UI.',
    onboarding_notice_system_unavailable: 'AVOI Agent todavía no está totalmente disponible. Bootstrap puede instalarlo, pero la configuración del proveedor quizá aún requiera una terminal.',
    onboarding_check_agent: 'AVOI Agent',
    ...
    onboarding_notice_setup_required: 'Elige aquí una ruta simple de proveedor. Los flujos OAuth avanzados siguen siendo del CLI de AVOI por ahora.',
    onboarding_notice_setup_already_ready: 'Ya se detectó una configuración funcional del proveedor de AVOI. Puedes conservarla o reemplazarla aquí.',
    onboarding_oauth_provider_ready_title: 'Proveedor ya autenticado',
    onboarding_oauth_provider_ready_body: 'Esta instancia está configurada para usar un proveedor OAuth (<strong>{provider}</strong>) configurado mediante la CLI de AVOI. No se necesita clave API aquí — haz clic en Continuar para finalizar la configuración.',
    onboarding_oauth_provider_not_ready_title: 'Proveedor OAuth no autenticado aún',
    onboarding_oauth_provider_not_ready_body: 'Esta instancia está configurada para usar <strong>{provider}</strong>, que utiliza OAuth en lugar de una clave API. Ejecuta <code>avoi auth</code> o <code>avoi model</code> en una terminal para autenticarte y recarga la interfaz web.',
    ...
    onboarding_api_key_help_prefix: 'Se guarda como secreto en tu archivo .env de AVOI usando',
    onboarding_base_url_label: 'Base URL',
    ...
    onboarding_workspace_help: 'Elige el modelo que AVOI debe usar para nuevos chats cuando termine la configuración.',
  ...
  de: {
    ...
    cmd_skills: 'Verfügbare AVOI-Skills auflisten',
  ...
  folder_add_as_space_title: 'Als Bereich hinzufügen?',
    ...
    settings_desc_cli_sessions: 'Fügt Sitzungen aus der AVOI CLI (state.db) in die Sitzungsliste ein. Klicken Sie auf eine CLI-Sitzung, um sie zu importieren und das Gespräch fortzusetzen.',
    ...
    settings_desc_bot_name: 'Anzeigename für den Assistenten in der UI. Standardmäßig AVOI.',
    ...
    onboarding_title: 'Willkommen bei AVOI',
    ...
    onboarding_step_system_desc: 'AVOI Agent muss installiert und erreichbar sein.',
    ...
    onboarding_notice_system_unavailable: 'Agent nicht erreichbar. Bitte starten Sie AVOI Agent.',
  ...
  folder_add_as_space_title: '\u6dfb\u52a0\u4e3a\u5de5\u4f5c\u533a\uff1f',
    ...
    settings_desc_cli_sessions: '将 AVOI CLI（state.db）中的会话合并到会话列表。点击某个 CLI 会话可导入并继续对话。',
    ...
    settings_desc_bot_name: '助手在 UI 中的显示名称。默认为 AVOI。',
    settings_desc_password: '输入新密码以设置或更改。留空保持当前设置。',
    // onboarding
    onboarding_badge: '首次运行',
    onboarding_title: '欢迎使用 AVOI WebUI',
    archive_extracted: (n, c) => `从 ${c} 个压缩包中解压了 ${n} 个文件`,
    onboarding_lead: '快速引导将验证 AVOI、保存真实的提供商配置、选择工作区和模型，并可选设置密码保护应用。',
    onboarding_back: '返回',
    onboarding_continue: '继续',
    onboarding_skip: '跳过设置',
    onboarding_skipped: '设置已跳过 — 使用现有配置。',
    onboarding_open: '打开 AVOI',
    onboarding_step_system_title: '系统检查',
    onboarding_step_system_desc: '验证 AVOI Agent 与配置可见性。',
    onboarding_step_setup_title: '提供商设置',
    onboarding_step_setup_desc: '保存最小可用的 AVOI 提供商配置。',
    ...
    onboarding_notice_system_ready: 'AVOI Agent 看起来可从 Web UI 访问。',
    onboarding_notice_system_unavailable: 'AVOI Agent 尚未完全可用。Bootstrap 可以安装它，但提供商设置可能仍需要终端。',
    onboarding_check_agent: 'AVOI Agent',
    ...
    onboarding_notice_setup_required: '请先在此选择一个简单的提供商路径。高级 OAuth 流程暂时仍建议在 AVOI CLI 中完成。',
    onboarding_notice_setup_already_ready: '已检测到可用的 AVOI 提供商配置。你可以保留它，或在这里替换。',
    onboarding_oauth_provider_ready_title: '提供商已完成认证',
    onboarding_oauth_provider_ready_body: '此实例已配置为使用通过 AVOI CLI 设置的 OAuth 提供商（<strong>{provider}</strong>）。这里不需要 API key，点击继续即可完成设置。',
    onboarding_oauth_provider_not_ready_title: 'OAuth 提供商尚未认证',
    onboarding_oauth_provider_not_ready_body: '此实例已配置为使用 <strong>{provider}</strong>，该提供商使用 OAuth 而非 API key。请在终端运行 <code>avoi auth</code> 或 <code>avoi model</code> 完成认证，然后重新加载 Web UI。',
  ...
  folder_add_as_space_title: '\u65b0\u589e\u70ba\u5de5\u4f5c\u5340\uff1f',
    ...
    settings_section_preferences_meta: 'AVOI WebUI 的預設值與介面行為。',
    ...
    settings_desc_cli_sessions: '將 AVOI CLI (的 state.db) 中的會話添加到會話清單。點擊一個 CLI 會話將導入它並繼續對話。',
    ...
    n_messages: (n) => `${n} 則訊息`,
    load_older_messages: '↑ 向上捲動或點擊以載入較早的訊息',
    onboarding_api_key_help_prefix: '\u900f\u904e\u4ee5\u4e0b\u65b9\u5f0f\u5132\u5b58\u70ba AVOI .env \u6a94\u6848\u4e2d\u7684\u6a5f\u5bc6',
    ...
    onboarding_lead: '\u5feb\u901f\u9010\u6b65\u8a2d\u5b9a\uff1a\u6aa2\u67e5 AVOI\u3001\u5132\u5b58\u63d0\u4f9b\u8005\u914d\u7f6e\u3001\u9078\u64c7\u5de5\u4f5c\u5340\u8207\u6a21\u578b\uff0c\u4e26\u53ef\u9078\u64c7\u52a0\u5165\u5bc6\u78bc\u4fdd\u8b77\u3002',
    ...
    onboarding_notice_setup_already_ready: '\u5df2\u5075\u6e2c\u5230\u53ef\u7528\u7684 AVOI \u63d0\u4f9b\u8005\u914d\u7f6e\u3002\u60a8\u53ef\u4ee5\u4fdd\u7559\u6216\u5728\u6b64\u53d6\u4ee3\u3002',
    onboarding_notice_setup_required: '\u5728\u6b64\u9078\u64c7\u5feb\u901f\u63d0\u4f9b\u8005\u8a2d\u5b9a\u8def\u5f91\u3002\u9032\u968e OAuth \u8a2d\u5b9a\u4ecd\u8acb\u4f7f\u7528 AVOI CLI\u3002',
    onboarding_notice_system_ready: 'AVOI Agent \u53ef\u5f9e Web UI \u5b58\u53d6\u3002',
    onboarding_notice_system_unavailable: 'AVOI Agent \u5c1a\u672a\u5b8c\u5168\u53ef\u7528\u3002\u5f15\u5c0e\u7a0b\u5f0f\u53ef\u80fd\u5b89\u88dd\u5b83\uff0c\u4f46\u63d0\u4f9b\u8005\u8a2d\u5b9a\u53ef\u80fd\u4ecd\u9700\u7d42\u7aef\u6a5f\u3002',
    onboarding_notice_workspace: '\u9019\u4e9b\u503c\u4f7f\u7528\u8207\u4e00\u822c\u61c9\u7528\u7a0b\u5f0f\u76f8\u540c\u7684\u8a2d\u5b9a API\u3002',
    onboarding_oauth_provider_not_ready_body: '\u6b64\u5be6\u4f8b\u8a2d\u5b9a\u4f7f\u7528 <strong>{provider}</strong>\uff0c\u5b83\u4f7f\u7528 OAuth \u800c\u975e API \u91d1\u9470\u3002\u8acb\u5728\u7d42\u7aef\u6a5f\u4e2d\u57f7\u884c <code>avoi auth</code> \u6216 <code>avoi model</code> \u9032\u884c\u6388\u6b0a\uff0c\u7136\u5f8c\u91cd\u65b0\u6574\u7406 Web UI\u3002',
    onboarding_oauth_provider_not_ready_title: 'OAuth \u63d0\u4f9b\u8005\u5c1a\u672a\u6388\u6b0a',
    onboarding_oauth_provider_ready_body: '\u6b64\u5be6\u4f8b\u8a2d\u5b9a\u4f7f\u7528\u900f\u904e AVOI CLI \u914d\u7f6e\u7684 OAuth \u63d0\u4f9b\u8005\uff08<strong>{provider}</strong>\uff09\u3002\u7121\u9700 API \u91d1\u9470 \u2014 \u6309\u7e7c\u7e8c\u5b8c\u6210\u8a2d\u5b9a\u3002',
    onboarding_oauth_provider_ready_title: '\u63d0\u4f9b\u8005\u5df2\u6388\u6b0a',
    onboarding_oauth_switch_hint: '\u6216\u9078\u64c7\u4e0b\u65b9\u5176\u4ed6\u63d0\u4f9b\u8005\u4ee5\u5207\u63db\u5230 API \u91d1\u9470\u8a2d\u5b9a\uff1a',
    onboarding_open: '\u958b\u555f AVOI',
    ...
    onboarding_step_setup_desc: '\u5132\u5b58\u6700\u5c0f\u53ef\u7528\u7684 AVOI \u63d0\u4f9b\u8005\u914d\u7f6e\u3002',
    onboarding_step_setup_title: '\u63d0\u4f9b\u8005\u8a2d\u5b9a',
    onboarding_step_system_desc: '\u9a57\u8b49 AVOI Agent \u8207\u914d\u7f6e\u53ef\u898b\u6027\u3002',
    onboarding_step_system_title: '\u7cfb\u7d71\u6aa2\u67e5',
    onboarding_step_workspace_desc: '\u70ba\u65b0\u6703\u8a71\u9078\u64c7\u9810\u8a2d\u503c\u3002',
    onboarding_step_workspace_title: '\u5de5\u4f5c\u5340\u8207\u6a21\u578b',
    onboarding_title: '\u6b61\u8fce\u4f7f\u7528 AVOI WebUI',
    onboarding_unknown: '\u672a\u77e5',
    onboarding_workspace_help: '\u9078\u64c7 AVOI \u5728\u8a2d\u5b9a\u5b8c\u6210\u5f8c\u7528\u65bc\u65b0\u804a\u5929\u5ba4\u7684\u6a21\u578b\u3002',
    ...
    providers_oauth_config_yaml_hint: '\u5df2\u900f\u904e config.yaml \u8a2d\u5b9a\u6b0a\u8b49\u3002\u8981\u66f4\u65b0\uff0c\u8acb\u7de8\u8f2f config.yaml \u4e2d\u7684 providers \u5340\u6bb5\uff0c\u6216\u57f7\u884c avoi auth\u3002',
    providers_oauth_not_configured_hint: '\u5c1a\u672a\u9a57\u8b49\u3002\u8acb\u5728\u7d42\u7aef\u6a5f\u57f7\u884c avoi auth \u4ee5\u8a2d\u5b9a\u6b64\u63d0\u4f9b\u8005\u3002',
    providers_remove: '\u79fb\u9664',
  ...
  pt: {
    ...
    cmd_skills: 'Listar skills disponíveis do AVOI',
    ...
    status_avoi_home: 'Diretório AVOI',
    ...
    settings_section_preferences_meta: 'Padrões e comportamento UI do AVOI WebUI.',
    ...
    settings_desc_cli_sessions: 'Mescla sessões do AVOI CLI na lista. Clique para importar.',
    ...
    settings_desc_bot_name: 'Nome de exibição do assistente. Padrão: AVOI.',
    ...
    providers_oauth_config_yaml_hint: 'Token configurado via config.yaml. Para atualizar, edite config.yaml ou rode avoi auth.',
    providers_oauth_not_configured_hint: 'Não autenticado. Rode avoi auth no terminal.',
    ...
    onboarding_title: 'Bem-vindo ao AVOI WebUI',
    onboarding_lead: 'Uma configuração rápida vai verificar AVOI, salvar provedor, escolher workspace e modelo, e opcionalmente proteger com senha.',
    ...
    onboarding_step_system_desc: 'Verificar AVOI Agent e visibilidade da config.',
    onboarding_step_setup_title: 'Configuração do provedor',
    onboarding_step_setup_desc: 'Salvar config mínima do provedor AVOI.',
    ...
    onboarding_notice_system_ready: 'AVOI Agent parece acessível pela Web UI.',
    onboarding_notice_system_unavailable: 'AVOI Agent não está totalmente disponível. Bootstrap pode instalar, mas setup do provedor pode requerer terminal.',
    onboarding_check_agent: 'AVOI Agent',
    ...
    onboarding_notice_setup_required: 'Escolha caminho simples de provedor aqui. OAuth avançado ainda pertence ao AVOI CLI.',
    onboarding_notice_setup_already_ready: 'Setup de provedor AVOI já detectado. Pode manter ou substituir.',
    ...
    onboarding_api_key_help_prefix: 'Salvo como segredo no .env do AVOI usando',
    onboarding_base_url_label: 'Base URL',
    ...
    onboarding_workspace_help: 'Escolha modelo que AVOI deve usar para novos chats.',
  ...
  ko: {
    ...
    settings_section_preferences_meta: 'AVOI WebUI의 기본값과 UI 동작.',
    ...
    settings_desc_cli_sessions: 'AVOI CLI(state.db)의 세션을 세션 목록에 병합합니다. CLI 세션을 클릭하면 가져와서 대화를 계속할 수 있습니다.',
    ...
    settings_desc_bot_name: 'UI 전체에 표시되는 Assistant 이름입니다. 기본값은 AVOI입니다.',
    ...
    onboarding_title: 'AVOI WebUI에 오신 것을 환영합니다',
    onboarding_lead: '간단한 안내 설정으로 AVOI 설치를 확인하고, 워크스페이스와 모델을 선택하며, 필요하면 비밀번호로 앱을 보호합니다.',
    onboarding_back: '뒤로',
...
function setLocale(lang) {
  ...
  localStorage.setItem('avoi-lang', resolved);
 ...
 */
function loadLocale() {
  setLocale(resolvePreferredLocale(null, localStorage.getItem('avoi-lang')));
}
