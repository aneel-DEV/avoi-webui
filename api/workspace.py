"""
AVOI Web UI -- Workspace and file system helpers.
...
def _profile_state_dir() -> Path:
    ...
    AVOI_WEBUI_STATE_DIR env var for test isolation).
    ...
    try:
        ...
        if name and name != 'default':
            d = get_active_avoi_home() / 'webui_state'
...
def _profile_default_workspace() -> str:
    ...
    Checks keys in priority order:
      ...
      3. 'terminal.cwd'      — avoi-agent terminal working dir (most common)
...
def _clean_workspace_list(workspaces: list) -> list:
    ...
    - Remove entries whose paths live inside another profile's directory
      (e.g. ~/.avoi/profiles/X/... should not appear on a different profile).
    - Rename any entry whose name is literally 'default' to 'Home' (avoids
      confusion with the 'default' profile name).
    Returns the cleaned list (may be empty).
    """
    avoi_profiles = (Path.home() / '.avoi' / 'profiles').resolve()
    result = []
    for w in workspaces:
        path = w.get('path', '')
        name = w.get('name', '')
        ...
        # created under ~/.avoi/profiles/webui/webui-mvp-test/).
        try:
            p.relative_to(avoi_profiles)
            # p is under ~/.avoi/profiles/ — only skip if it's under a DIFFERENT profile
            try:
                from api.profiles import get_active_avoi_home
                own_profile_dir = get_active_avoi_home().resolve()
...
def resolve_trusted_workspace(path: str | Path | None = None) -> Path:
    ...
    #     In Docker deployments AVOI_WEBUI_DEFAULT_WORKSPACE is often set to a
